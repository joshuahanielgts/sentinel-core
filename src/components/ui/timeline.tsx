...
export function Timeline({ data, eyebrow, title, description, className }: TimelineProps) {
  ...
  return (
    <div className={cn('w-full font-sans md:px-10', className)} ref={containerRef}>
      {(eyebrow || title || description) && (
        <div className="max-w-7xl mx-auto pb-10 px-4 md:px-8 lg:px-10">
          {eyebrow && (
            <p className="text-xs font-mono text-accent tracking-widest mb-3 uppercase">
              {eyebrow}
            </p>
          )}
          {title && (
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-32 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-muted-foreground/70 font-mono">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-muted-foreground/70 font-mono">
                {item.title}
              </h3>
              {item.content}
            </div>
